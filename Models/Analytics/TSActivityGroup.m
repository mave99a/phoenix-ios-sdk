...
#import "TSActivityGroup.h"
#import "TSPhoenixClient.h"

@implementation TSActivityGroup
...
+ (NSDictionary *)mappingDictionary {
	return @{
		...
		@"ActivityGroup2" : @{@"type": @"relationship", @"mappedType":@"TSActivityGroup2", @"mappedName": @"activityGroup2"}
	};
}

- (NSString *)dbKey {
	
    return [[self class] dbKeyWithID:self.activityGroupID];
	
}

+ (NSString *)dbKeyWithID: (NSNumber *)identifier {
    return [NSString stringWithFormat:@"Phoenix/ActivityGroup/%@", [identifier stringValue]];
}
...
+ (NSString *)dbCollection {
  ...
  return @"Phoenix/ActivityGroup";
  
}

+ (instancetype)activityGroupWithID: (NSNumber *)objectID {
